...
export function PlayerProfileView({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Tournament by tournament: the shape of a season, not just its average. */}
      <div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-x-auto">
        <table className="w-full text-xs border-collapse min-w-[520px]">
          <thead>
            <tr className="bg-slate-100/70 border-b border-slate-200 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              <th className="py-1.5 px-3 text-left">Tournament</th>
              <th className="py-1.5 px-2 text-right">Matches</th>
              <th className="py-1.5 px-2 text-right">Pts / match</th>
              <th className="py-1.5 px-2 text-right">Kill %</th>
              <th className="py-1.5 px-2 text-right">Efficiency</th>
              <th className="py-1.5 px-2 text-right">Blocks</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {profile.tournaments.map((row) => {
              const values = metricValues(row.totals);
              return (
                <tr key={row.tournamentNo} className="hover:bg-slate-50/70">
                  <td className="py-1.5 px-3">
                    <Link
                      href={`/tournaments/${row.tournamentNo}`}
                      className="font-semibold text-slate-900 hover:text-amber-600 transition-colors"
                    >
                      {row.title}
                    </Link>
                    <span className="ml-1.5 text-[10px] text-slate-400">{row.startDate}</span>
                  </td>
                  <td className="py-1.5 px-2 text-right font-mono tabular-nums text-slate-600">
                    {row.totals.matches}
                  </td>
                  <td className="py-1.5 px-2 text-right font-mono tabular-nums font-bold text-slate-800">
                  ...
                  <td
                    className={`py-1.5 px-2 text-right font-mono tabular-nums ${
                      (values.spikeEfficiency ?? 0) < 0 ? "text-red-600 font-bold" : "text-slate-600"
                    }`}
                  >
                    {values.spikeEfficiency === null ? "—" : `${values.spikeEfficiency.toFixed(1)}%`}
                  </td>
                  <td className="py-1.5 px-2 text-right font-mono tabular-nums text-slate-600">
                    {row.totals.blockPoint}
                  </td>
                </tr>
              );
            })}
